import {NuxtPayload, useRoute, useRouter} from '#app';
import {
    ILifeCycle,
    IRouterable,
    onActivated,
...
export class ViewModelFactory<T extends ClassInstanceType, G extends InstanceType<T> = InstanceType<T>> {
    ...
    private async callComponentHooks() {
        ...
        if (this.store['onSetup'])
            await this.store['onSetup']?.apply(this.store);
        ...
        onUnmounted(async () => {
            if (!this.store || !this.pinia) {
                return;
            }

            if (this.store['onUnmounted'])
                await this.store['onUnmounted'].apply(this.store);
    ...
    private static findVueHook(name: keyof StoreDefinition) {
        switch (name) {
        case 'onMounted':
            return onMounted;
        case 'onUnmounted':
